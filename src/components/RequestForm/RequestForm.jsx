...
export default function Test() {
  ...
  const handleChangeClinics = (event) => {
    ...
    if (event.target.value !== "") {
      if (event.key === "Enter") {
        ...
        let urlForMap = `https://maps.googleapis.com/maps/api/place/findplacefromtext/json?input=${nameOfClinic.value}&inputtype=textquery&fields=name,formatted_address,rating,opening_hours,geometry,place_id&key=${apiKey}`;
  ...
  const selectedCountryData = useMemo(() => {
    return (
      data.paises.find((country) => country.name === selectedCountry) || {}
    );
  ...
  useEffect(() => {
    ...
    if (selectedCountry) {
      const countryData = data.paises.find(
                ...
                clinicsToDisplay.map((clinic) => {
                  return (
                    <MediaCard
                      ...
                      component="img"
                      photo={
                        clinic.photos ? clinic.photos[0] : "no-clinic.webp"
                      }
                    />
                  );
                })}
